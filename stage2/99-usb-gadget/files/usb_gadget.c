 ...
 *
 * The directory must already be a mounted functionfs instance
 * (see gadget_setup.sh).  Defaults to /dev/ffs-loopback.
 */
...
/* 7-byte endpoint descriptor (no audio extension). */
struct ep_desc {
    uint8_t  bLength;
...
/* Returns: 1 = ENABLE received, -2 = DISABLE received,              */
/*         -1 = fatal error,      0 = other / ignorable              */
/* ------------------------------------------------------------------ */
...
static int process_ep0_event(int ep0)
{
    ...
    case FUNCTIONFS_SETUP:
        /* Stall any setup requests we don't handle. */
...
int main(int argc, char *argv[])
{
    ...
    /* Write USB descriptors, then string descriptors. */
    ...
    while (running) {
        struct pollfd fds[2] = {
            ...
            { .fd = ep_out, .events = POLLIN }, /* fd=-1 is ignored by poll */
        };
        ...
        /* Handle control endpoint events first. */
        if (fds[0].revents & POLLIN) {
            int rc = process_ep0_event(ep0);
            if (rc == 1) {
                /* Host enabled the interface – open data endpoints. */
                ...
                /* Host disabled the interface – close data endpoints. */
                close(ep_in);  ep_in  = -1;
