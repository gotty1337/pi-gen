...
# ── Wait for daemon to write USB descriptors (ep1 appears after descriptors written)
for i in $(seq 1 20); do
    [ -e "$FFSDIR/ep1" ] && break
    sleep 0.5
done
if [ ! -e "$FFSDIR/ep1" ]; then
    echo "ERROR: Daemon did not write FunctionFS descriptors in time" >&2
